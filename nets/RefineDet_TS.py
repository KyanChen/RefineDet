...
# Tiny_Scale 检测微小目标的网络[128, 64, 32, 16, 8]
...
def getNumPriorBoxesEachLayer():
    numBoxes = np.ones(len(Config.CFG['feature_maps']), dtype=np.int)
...
class RefineSSD(nn.Module):
    ...
    def __init__(self, num_classes, is_refine=False):
        ...
        self.odm_loc = nn.ModuleList([
            ...
            nn.Conv2d(256, self.numPriorBoxesEachLayer[4] * 4, kernel_size=3, stride=1, padding=1),
        ])
        self.odm_conf = nn.ModuleList([
            ...
            nn.Conv2d(256, self.numPriorBoxesEachLayer[4] * self.num_classes, kernel_size=3, stride=1, padding=1),
        ])
    ...
    def forward(self, x):
        ...
        for k in range(16 + self.offset[0]):
        ...
        for k in range(16 + self.offset[0], 23 + self.offset[1]):
            x = self.base_network[k](x)
        ...
        for k in range(23 + self.offset[1], 30 + self.offset[2]):
            x = self.base_network[k](x)
        ...
        # apply vgg up to fc7
        for k in range(30 + self.offset[2], len(self.base_network)):
            x = self.base_network[k](x)
        ...
        # apply multibox head to arm branch, arm分支的cls + loc预测，刚好对应arm_sources新增的5个分支
        if self.is_refine:
